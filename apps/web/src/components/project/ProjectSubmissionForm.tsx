...
import GameUploadWidget from '../GameUploadWidget';
...
export function ProjectSubmissionForm({ mode }: ProjectSubmissionFormProps) {
  ...
  }, [fields.length, getValues, isAdminMode, setValue, user]);
  ...
  const onSubmit = (data: SubmitProjectPayloadInput) => {
    // Admin submissions may link a matching member row; user submissions bind creator server-side.
    if (isAdminMode && user) {
      const linkedMember = data.members.find((m) => m.name === user.name);
