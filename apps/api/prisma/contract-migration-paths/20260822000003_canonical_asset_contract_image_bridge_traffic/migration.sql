...
-- Explicit alternate history: observation age and canonical image bridge traffic counts are waived.
...
BEGIN
  IF (SELECT count(*) FROM "release_contract_authorizations"
      WHERE "migration_name" = '20260822000003_canonical_asset_contract_image_bridge_traffic'
        AND "scope" = '24-hour-observation-age-and-image-bridge-traffic'
...
FROM "release_contract_authorizations" WHERE "migration_name" = '20260822000003_canonical_asset_contract_image_bridge_traffic';
...
BEGIN
  ...
  IF TRUE THEN -- An explicit exception requires all seven observations even on an empty database.
    ...
    AND metric."value" > 0 AND metric."value" <= 9007199254740991
