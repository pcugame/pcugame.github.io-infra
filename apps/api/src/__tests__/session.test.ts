...
describe('session helpers', () => {
	...
	it('calculates absolute session expiry from the configured window', () => {
		...
		expect(absoluteSessionExpiresAt(from, SESSION_ABSOLUTE_MS).toISOString())
	...
	it('uses idle expiry for cookies when idle ends before absolute expiry', () => {
		...
		expect(cookieExpiresAt(session, now, SESSION_IDLE_MS).toISOString())
	...
	it('treats sessions as idle-expired only after the idle window is exceeded', () => {
		...
		expect(isIdleExpired(new Date('2026-01-01T00:00:00.000Z'), now, SESSION_IDLE_MS)).toBe(false);
		expect(isIdleExpired(new Date('2025-12-31T23:59:59.999Z'), now, SESSION_IDLE_MS)).toBe(true);
	});
});
