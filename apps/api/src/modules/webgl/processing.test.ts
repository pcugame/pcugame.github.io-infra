...
describe('canonical WebGL processing', () => {
	it.each(['array', 'object'] as const)('retains the historical %s source manifest while accepting allocator base64', async (format) => {
		const state = await harness(unityZip());
		const bytes = Buffer.from(state.uploadSession.sourceIdentityBlockManifest as string, 'base64');
		const digests = [];
		for (let offset = 0; offset < bytes.length; offset += 32) digests.push(bytes.subarray(offset, offset + 32).toString('hex'));
		state.uploadSession.sourceIdentityBlockManifest = format === 'array' ? digests : { digests };
		await state.processor.process(state.uploadSession, state.context);
		expect(state.repository.commitReady).toHaveBeenCalled();
	});

	it.each(['!', 'YWJj=', ''])('rejects malformed persisted base64 %j before publication', async (manifest) => {
		const state = await harness(unityZip());
		state.uploadSession.sourceIdentityBlockManifest = manifest;
