...
async function main() {
	...
	try {
		const assets = await resources.prisma.asset.findMany({
			where: { status: 'READY' },
		...
		for (const asset of assets) {
			const bucket = bucketForKind(cfg, asset.kind);
			...
			const existing = await resources.storage.head(bucket, asset.storageKey);
			if (existing) {
				skipped++;
				continue;
			}
