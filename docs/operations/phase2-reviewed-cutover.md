...
현재 Pages 계정 권한은 `push=true`, `admin=false`이다. 보호 설정 조회는 404를 반환하고 `PAGES_DEPLOY_ACTOR`는 미확인이다. 이 제약은 관측 기간 예외로 면제하지 않는다. Pages 검사는 유지보수 전에 실행하므로 실패하면 서비스 중지·DB 전환을 시작하지 않는다.
